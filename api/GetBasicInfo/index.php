...
try {
    ...
    if (!in_array('user.basic', $permissions)) {
        $logger->warning('api', '权限不足', [
            'app_id' => $appId,
            'openid' => $openid,
            'required_permission' => 'user.basic',
            'current_permissions' => $permissions
        ]);
        jsonResponse(false, null, '权限不足，需要 user.basic 权限', 403);
    }
    ...
    error_log("获取用户基本信息错误: " . $e->getMessage());
    
    // 尝试记录到系统日志
    try {
        if (isset($pdo) && $pdo && isset($logger)) {
            $logger->critical('api', '获取用户基本信息异常', [
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ], $e->getTraceAsString());
