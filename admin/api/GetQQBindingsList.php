...
try {
    ...
    if (!$tableCheck['table_exists']) {
        $logger->error('admin', 'QQ 用户信息表不存在', [
            'admin' => $admin['username']
        ]);
        jsonResponse(false, null, '数据库表 auth.qq_user_info 不存在，请先执行 SQL 脚本创建表', 500);
    }
    
    // 检查是否有数据
    $countCheckSql = "SELECT COUNT(*) as total FROM auth.qq_user_info";
    $stmt = $pdo->query($countCheckSql);
    $dataCheck = $stmt->fetch(PDO::FETCH_ASSOC);
    
    $logger->info('admin', 'QQ 绑定表状态检查', [
        'admin' => $admin['username'],
        'table_exists' => true,
    ...
    if (!empty($search)) {
        $whereConditions[] = "(q.openid LIKE :search OR q.qq_nickname LIKE :search OR u.username LIKE :search OR u.nickname LIKE :search)";
        $params[':search'] = '%' . $search . '%';
    }
    
    if ($bindStatus >= 0) {
        $whereConditions[] = "q.bind_status = :bind_status";
    ...
    $countSql = "
        ...
        FROM auth.qq_user_info q
        LEFT JOIN users.user u ON q.user_uuid = u.uuid
        $whereClause
    ";
    ...
    foreach ($bindings as $binding) {
        $formattedBindings[] = [
            ...
            'qq_avatar' => $binding['qq_avatar'],
            'qq_gender' => $binding['qq_gender'],
    ...
    // 记录日志
    $logger->info('admin', '管理员查看 QQ 绑定列表', [
        'admin' => $admin['username'],
        'page' => $page,
        'page_size' => $pageSize,
        'search' => $search
    ...
    error_log('获取 QQ 绑定列表失败: ' . $e->getMessage());
    ...
    if (isset($logger)) {
        $logger->error('admin', '获取 QQ 绑定列表失败：数据库错误', [
            'error' => $e->getMessage(),
            'sql_state' => $e->getCode(),
    ...
    if (isset($logger)) {
        $logger->error('admin', '获取 QQ 绑定列表失败：系统错误', [
            'error' => $e->getMessage()
        ]);
    }
