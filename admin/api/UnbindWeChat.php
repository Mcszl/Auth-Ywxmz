...
try {
    ...
    if ($binding['bind_status'] == 0) {
        jsonResponse(false, null, '该微信账号已经是解绑状态', 400);
    }
    ...
    error_log('解绑微信失败: ' . $e->getMessage());
    
    if (isset($logger)) {
        $logger->error('admin', '解绑微信失败：数据库错误', [
            'error' => $e->getMessage()
        ]);
    }
    ...
    if (isset($logger)) {
        $logger->error('admin', '解绑微信失败：系统错误', [
            'error' => $e->getMessage()
        ]);
    }
