...
try {
    ...
    $limitType = isset($_GET['limit_type']) ? trim($_GET['limit_type']) : '';
    ...
    // 搜索条件
    if (!empty($search)) {
        $conditions[] = "(limit_name ILIKE :search OR description ILIKE :search)";
    ...
    // 查询总数
    $countSql = "SELECT COUNT(*) FROM sms.send_limit $whereClause";
