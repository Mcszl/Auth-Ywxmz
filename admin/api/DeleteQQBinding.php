...
try {
    ...
    // 查询绑定信息
    $stmt = $pdo->prepare("
        SELECT id, openid, user_uuid, qq_nickname
    ...
    // 删除绑定记录
    $stmt = $pdo->prepare("
        DELETE FROM auth.qq_user_info
        WHERE id = :id
    ");
    $stmt->execute([':id' => $bindingId]);
    
    // 记录日志
    $logger->info('admin', '管理员删除 QQ 绑定', [
    ...
    error_log('删除 QQ 绑定失败: ' . $e->getMessage());
    
    if (isset($logger)) {
        $logger->error('admin', '删除 QQ 绑定失败：数据库错误', [
            'error' => $e->getMessage()
        ]);
    }
    ...
    if (isset($logger)) {
        $logger->error('admin', '删除 QQ 绑定失败：系统错误', [
            'error' => $e->getMessage()
        ]);
    }
