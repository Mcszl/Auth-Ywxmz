...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, google_id, user_uuid, bind_status 
    ...
    if ($binding['bind_status'] == 0) {
        jsonResponse(false, null, '该 Google 账号已经是未绑定状态', 400);
    }
    ...
    try {
        $updateStmt = $pdo->prepare("
            ...
            WHERE id = :id
        ");
        $updateStmt->execute(['id' => $bindingId]);
        
        $pdo->commit();
        
        $logger->info('admin', '管理员解绑 Google 账号', [
            'admin' => $admin['username'],
            'google_id' => $binding['google_id'],
            'original_user_uuid' => $binding['user_uuid']
        ]);
