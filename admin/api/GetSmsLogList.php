...
/**
 * 获取短信日志列表 API
 * 支持日期筛选、手机号筛选、用途筛选、状态筛选、渠道筛选、分页等功能
 */
...
try {
    ...
    // 关键词搜索
    if (!empty($keyword)) {
        $conditions[] = "(sms_id ILIKE :keyword OR upstream_sms_id ILIKE :keyword)";
    ...
    jsonResponse(true, [
        ...
        'stats' => [
            ...
            'unique_phone_count' => intval($stats['unique_phone_count']),
    ...
    error_log("获取短信日志失败: " . $e->getMessage());
    jsonResponse(false, null, '获取日志失败', 500);
} catch (Exception $e) {
    error_log("获取短信日志错误: " . $e->getMessage());
