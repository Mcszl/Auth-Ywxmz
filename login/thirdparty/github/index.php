...
try {
    ...
    if (!$app) {
        $logger->log('error', 'github_login_app_not_found', 'GitHub登录失败：应用不存在', ['app_id' => $appId]);
        throw new Exception('应用不存在');
    }
    
    // 检查应用状态
    if ($app['status'] == 0) {
        $logger->log('warning', 'github_login_app_banned', 'GitHub登录失败：应用已被封禁', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    if ($app['status'] == 2) {
        $logger->log('warning', 'github_login_app_pending', 'GitHub登录失败：应用正在审核中', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    // 检查是否启用登录功能
    if (!$app['enable_login']) {
        $logger->log('warning', 'github_login_disabled', 'GitHub登录失败：应用未启用登录功能', ['app_id' => $appId]);
        throw new Exception('该应用未启用登录功能');
    }
    
    // 检查是否开启第三方登录
    if (!$app['enable_third_party_login']) {
        $logger->log('warning', 'github_login_third_party_disabled', 'GitHub登录失败：应用未开启第三方登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启第三方登录功能');
    }
    
    // 检查是否开启GitHub登录
    if (!$app['enable_github_login']) {
        $logger->log('warning', 'github_login_platform_disabled', 'GitHub登录失败：应用未开启GitHub登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启GitHub登录功能');
    }
    ...
    if (!$callbackValid) {
        $logger->log('warning', 'github_login_callback_invalid', 'GitHub登录失败：回调地址未授权', [
            'app_id' => $appId,
    ...
    if (!empty($permissions)) {
        ...
        if (!empty($invalidPermissions)) {
            $logger->log('warning', 'github_login_permissions_invalid', 'GitHub登录失败：请求的权限未授权', [
                'app_id' => $appId,
    ...
    if (!$config) {
        $logger->log('error', 'github_login_config_not_found', 'GitHub登录失败：GitHub登录配置未启用或不存在', ['app_id' => $appId]);
    ...
    // GitHub OAuth授权地址
    $authUrl = $extraConfig['auth_url'] ?? 'https://github.com/login/oauth/authorize';
    ...
    $_SESSION['github_oauth_time'] = time();
    $_SESSION['github_oauth_app_id'] = $appId;
    $_SESSION['github_oauth_callback_url'] = $callbackUrl;
    $_SESSION['github_oauth_permissions'] = $permissions;
    $_SESSION['github_oauth_state_code'] = $stateCode;
    
    // 记录日志
    $logger->log('info', 'github_login_start', 'GitHub登录发起授权请求', [
        'app_id' => $appId,
