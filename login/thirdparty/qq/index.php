...
try {
    ...
    if (!$app) {
        $logger->log('error', 'qq_login_app_not_found', 'QQ登录失败：应用不存在', ['app_id' => $appId]);
        throw new Exception('应用不存在');
    }
    
    // 检查应用状态
    if ($app['status'] == 0) {
        $logger->log('warning', 'qq_login_app_banned', 'QQ登录失败：应用已被封禁', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    if ($app['status'] == 2) {
        $logger->log('warning', 'qq_login_app_pending', 'QQ登录失败：应用正在审核中', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    // 检查是否启用登录功能
    if (!$app['enable_login']) {
        $logger->log('warning', 'qq_login_disabled', 'QQ登录失败：应用未启用登录功能', ['app_id' => $appId]);
        throw new Exception('该应用未启用登录功能');
    }
    
    // 检查是否开启第三方登录
    if (!$app['enable_third_party_login']) {
        $logger->log('warning', 'qq_login_third_party_disabled', 'QQ登录失败：应用未开启第三方登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启第三方登录功能');
    }
    
    // 检查是否开启QQ登录
    if (!$app['enable_qq_login']) {
        $logger->log('warning', 'qq_login_platform_disabled', 'QQ登录失败：应用未开启QQ登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启QQ登录功能');
    }
    ...
    if (!$callbackValid) {
        $logger->log('warning', 'qq_login_callback_invalid', 'QQ登录失败：回调地址未授权', [
            'app_id' => $appId,
    ...
    if (!empty($permissions)) {
        ...
        if (!empty($invalidPermissions)) {
            $logger->log('warning', 'qq_login_permissions_invalid', 'QQ登录失败：请求的权限未授权', [
                'app_id' => $appId,
    ...
    if (!$config) {
        $logger->log('error', 'qq_login_config_not_found', 'QQ登录失败：QQ登录配置未启用或不存在', ['app_id' => $appId]);
    ...
    // QQ互联授权地址
    $authUrl = $extraConfig['auth_url'] ?? 'https://graph.qq.com/oauth2.0/authorize';
    ...
    $_SESSION['qq_oauth_time'] = time();
    $_SESSION['qq_oauth_app_id'] = $appId;
    $_SESSION['qq_oauth_callback_url'] = $callbackUrl;
    $_SESSION['qq_oauth_permissions'] = $permissions;
    $_SESSION['qq_oauth_state_code'] = $stateCode;
    
    // 记录日志
    $logger->log('info', 'qq_login_start', 'QQ登录发起授权请求', [
        'app_id' => $appId,
    ...
    $params = [
        ...
        'state' => $state,
        'scope' => $config['scopes'] ?? 'get_user_info'
    ];
