...
try {
    ...
    if (isset($_SESSION['qq_bind_state']) && $state === $_SESSION['qq_bind_state']) {
        // 绑定模式
        $isBindMode = true;
        
        // 检查state是否过期（10分钟有效期）
        if (!isset($_SESSION['qq_bind_time']) || (time() - $_SESSION['qq_bind_time']) > 600) {
            throw new Exception('授权已过期，请重新操作');
        }
        
        // 检查是否已登录
        if (!isset($_SESSION['user_uuid'])) {
            throw new Exception('请先登录后再绑定QQ账号');
        }
        
        // 清除session中的state
        unset($_SESSION['qq_bind_state']);
        unset($_SESSION['qq_bind_time']);
        
    } elseif (isset($_SESSION['qq_oauth_state']) && $state === $_SESSION['qq_oauth_state']) {
        // 登录模式
        $isBindMode = false;
        
        // 检查state是否过期（10分钟有效期）
        if (!isset($_SESSION['qq_oauth_time']) || (time() - $_SESSION['qq_oauth_time']) > 600) {
            throw new Exception('授权已过期，请重新登录');
        }
        
        // 从session中获取所有必要参数
        $appId = $_SESSION['qq_oauth_app_id'] ?? '';
        $callbackUrl = $_SESSION['qq_oauth_callback_url'] ?? '';
        $permissions = $_SESSION['qq_oauth_permissions'] ?? '';
        $stateCode = $_SESSION['qq_oauth_state_code'] ?? '';
    ...
    // QQ互联Token接口地址
    $tokenUrl = $extraConfig['token_url'] ?? 'https://graph.qq.com/oauth2.0/token';
    ...
    if (!$tokenData || isset($tokenData['error'])) {
        $errorMsg = $tokenData['error_description'] ?? '获取Access Token失败';
    ...
    // 获取OpenID
    $openidUrl = $extraConfig['openid_url'] ?? 'https://graph.qq.com/oauth2.0/me';
    $openidResponse = file_get_contents($openidUrl . '?access_token=' . $accessToken . '&fmt=json');
    
    if ($openidResponse === false) {
    ...
    if (!$openidData || isset($openidData['error'])) {
        $errorMsg = $openidData['error_description'] ?? '获取OpenID失败';
        throw new Exception($errorMsg);
    }
    
    $openid = $openidData['openid'] ?? '';
    $clientId = $openidData['client_id'] ?? '';
    ...
    // 获取用户信息
    $userinfoUrl = $extraConfig['userinfo_url'] ?? 'https://graph.qq.com/user/get_user_info';
    $userinfoParams = [
    ...
    if (!$userinfo || $userinfo['ret'] !== 0) {
        $errorMsg = $userinfo['msg'] ?? '获取用户信息失败';
        throw new Exception($errorMsg);
    }
    ...
    $qqAvatar = $userinfo['figureurl_qq_2'] ?? $userinfo['figureurl_qq_1'] ?? $userinfo['figureurl'] ?? '';
    ...
    if ($isBindMode) {
        ...
        if ($existingBind && $existingBind['bind_status'] == 1 && $existingBind['user_uuid'] != $userUuid) {
            throw new Exception('该QQ账号已被其他用户绑定');
        }
        ...
        if ($currentBind) {
            throw new Exception('您已经绑定了其他QQ账号，请先解绑');
        }
        ...
        try {
            ...
            <body>
                <div class="success-container">
                    ...
                    <h1>QQ账号绑定成功</h1>
                    <?php if (!empty($qqAvatar)): ?>
                    <img src="<?php echo htmlspecialchars($qqAvatar); ?>" alt="头像" class="avatar">
                    <?php endif; ?>
                    <div class="nickname"><?php echo htmlspecialchars($qqNickname); ?></div>
    ...
    $stmt = $pdo->prepare("
        ...
        FROM auth.qq_user_info qi
        LEFT JOIN users.user u ON qi.user_uuid = u.uuid
        WHERE qi.openid = :openid
    ");
    
    $stmt->execute(['openid' => $openid]);
    $qqUser = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($qqUser && $qqUser['bind_status'] == 1 && !empty($qqUser['user_uuid'])) {
        // ============================================
        // 4. 已绑定账号，执行登录流程
        // ============================================
        
        // 检查用户状态
        if ($qqUser['user_status'] != 1) {
            $statusText = ['0' => '已封禁', '2' => '待验证'][$qqUser['user_status']] ?? '状态异常';
        ...
        // 验证是否启用QQ登录
        if (!$appConfig['enable_qq_login']) {
            throw new Exception('应用未启用QQ登录功能');
        }
        ...
        ");
        $stmt->execute(['uuid' => $qqUser['user_uuid']]);
        ...
        // 清除session中的OAuth参数
        unset($_SESSION['qq_oauth_app_id']);
        unset($_SESSION['qq_oauth_callback_url']);
        unset($_SESSION['qq_oauth_permissions']);
        unset($_SESSION['qq_oauth_state_code']);
        ...
        // 临时存储QQ用户信息到session，供绑定页面使用
        $_SESSION['qq_temp_info'] = [
            'openid' => $openid,
            'nickname' => $qqNickname,
            'avatar' => $qqAvatar,
            'gender' => $qqGender
    ...
    <body>
        <div class="notice-container">
            ...
            <div class="notice-text">
                <strong>提示：</strong>您的QQ账号尚未绑定本平台账号。<br><br>
                请先登录用户中心，在"账号安全"页面绑定QQ账号后，即可使用QQ快捷登录功能。
            </div>
            
            <div class="user-info">
                <div class="user-info-item">
                    <span class="user-info-label">QQ昵称</span>
                    <span class="user-info-value"><?php echo htmlspecialchars($qqNickname); ?></span>
                </div>
                <div class="user-info-item">
                    <span class="user-info-label">性别</span>
                    <span class="user-info-value"><?php echo $qqGender === '男' ? '男' : ($qqGender === '女' ? '女' : '未知'); ?></span>
                </div>
            </div>
    ...
    if (isset($logger)) {
        $logger->log(
            'error',
            'qq_login_callback_error',
            'QQ登录回调失败: ' . $e->getMessage(),
            [
                'code' => $_GET['code'] ?? '',
                'state' => $_GET['state'] ?? '',
                'error' => $e->getMessage(),
    ...
    if (isset($isBindMode) && $isBindMode) {
        ...
        if (isset($_SESSION['qq_oauth_app_id']) && !empty($_SESSION['qq_oauth_app_id'])) {
            $params = [
                'app_id' => $_SESSION['qq_oauth_app_id'],
                'callback_url' => $_SESSION['qq_oauth_callback_url'] ?? '',
                'permissions' => $_SESSION['qq_oauth_permissions'] ?? '',
                'code' => $_SESSION['qq_oauth_state_code'] ?? ''
            ];
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>QQ<?php echo isset($isBindMode) && $isBindMode ? '绑定' : '登录'; ?>失败</title>
        <style>
            body {
                font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;
                background: #f5f5f5;
    ...
    <body>
        <div class="error-container">
            ...
            <h1>QQ<?php echo isset($isBindMode) && $isBindMode ? '绑定' : '登录'; ?>失败</h1>
