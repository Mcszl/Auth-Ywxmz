...
try {
    ...
    if (!$app) {
        $logger->log('error', 'wechat_login_app_not_found', '微信登录失败：应用不存在', ['app_id' => $appId]);
        throw new Exception('应用不存在');
    }
    
    // 检查应用状态
    if ($app['status'] == 0) {
        $logger->log('warning', 'wechat_login_app_banned', '微信登录失败：应用已被封禁', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    if ($app['status'] == 2) {
        $logger->log('warning', 'wechat_login_app_pending', '微信登录失败：应用正在审核中', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    // 检查是否启用登录功能
    if (!$app['enable_login']) {
        $logger->log('warning', 'wechat_login_disabled', '微信登录失败：应用未启用登录功能', ['app_id' => $appId]);
        throw new Exception('该应用未启用登录功能');
    }
    
    // 检查是否开启第三方登录
    if (!$app['enable_third_party_login']) {
        $logger->log('warning', 'wechat_login_third_party_disabled', '微信登录失败：应用未开启第三方登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启第三方登录功能');
    }
    
    // 检查是否开启微信登录
    if (!$app['enable_wechat_login']) {
        $logger->log('warning', 'wechat_login_platform_disabled', '微信登录失败：应用未开启微信登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启微信登录功能');
    }
    ...
    if (!$callbackValid) {
        $logger->log('warning', 'wechat_login_callback_invalid', '微信登录失败：回调地址未授权', [
            'app_id' => $appId,
    ...
    if (!empty($permissions)) {
        ...
        if (!empty($invalidPermissions)) {
            $logger->log('warning', 'wechat_login_permissions_invalid', '微信登录失败：请求的权限未授权', [
                'app_id' => $appId,
    ...
    if (!$config) {
        $logger->log('error', 'wechat_login_config_not_found', '微信登录失败：微信登录配置未启用或不存在', ['app_id' => $appId]);
    ...
    // 微信开放平台授权地址
    // 注意：微信有两种登录方式
    // 1. 网站应用微信登录（PC端）：https://open.weixin.qq.com/connect/qrconnect
    // 2. 公众号网页授权（移动端）：https://open.weixin.qq.com/connect/oauth2/authorize
    ...
    $_SESSION['wechat_oauth_time'] = time();
    $_SESSION['wechat_oauth_app_id'] = $appId;
    $_SESSION['wechat_oauth_callback_url'] = $callbackUrl;
    $_SESSION['wechat_oauth_permissions'] = $permissions;
    $_SESSION['wechat_oauth_state_code'] = $stateCode;
    
    // 记录日志
    $logger->log('info', 'wechat_login_start', '微信登录发起授权请求', [
        'app_id' => $appId,
    ...
    // 构建授权URL参数
    $params = [
        'appid' => $config['app_id'],  // 微信使用appid而不是client_id
        'redirect_uri' => $config['callback_url'],  // 不需要手动urlencode，http_build_query会自动处理
        'response_type' => 'code',
        'scope' => $scopes,  // 微信网站应用默认使用snsapi_login
        'state' => $state
    ];
    
    // 构建完整的授权URL
    // 微信的URL格式与QQ不同，需要添加#wechat_redirect后缀
    ...
    <body>
        <div class="error-container">
            ...
            <a href="../../index.html" class="btn-back">返回登录页</a>
        </div>
    </body>
    </html>
    <?php
