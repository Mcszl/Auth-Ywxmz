...
try {
    ...
    if (!$app) {
        $logger->log('error', 'google_login_app_not_found', 'Google登录失败：应用不存在', ['app_id' => $appId]);
        throw new Exception('应用不存在');
    }
    
    // 检查应用状态
    if ($app['status'] == 0) {
        $logger->log('warning', 'google_login_app_banned', 'Google登录失败：应用已被封禁', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    if ($app['status'] == 2) {
        $logger->log('warning', 'google_login_app_pending', 'Google登录失败：应用正在审核中', [
            'app_id' => $appId,
            'status' => $app['status']
        ]);
    ...
    // 检查是否启用登录功能
    if (!$app['enable_login']) {
        $logger->log('warning', 'google_login_disabled', 'Google登录失败：应用未启用登录功能', ['app_id' => $appId]);
        throw new Exception('该应用未启用登录功能');
    }
    
    // 检查是否开启第三方登录
    if (!$app['enable_third_party_login']) {
        $logger->log('warning', 'google_login_third_party_disabled', 'Google登录失败：应用未开启第三方登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启第三方登录功能');
    }
    
    // 检查是否开启Google登录
    if (!$app['enable_google_login']) {
        $logger->log('warning', 'google_login_platform_disabled', 'Google登录失败：应用未开启Google登录', ['app_id' => $appId]);
        throw new Exception('该应用未开启Google登录功能');
    }
    ...
    if (!$callbackValid) {
        $logger->log('warning', 'google_login_callback_invalid', 'Google登录失败：回调地址未授权', [
            'app_id' => $appId,
    ...
    if (!empty($permissions)) {
        ...
        if (!empty($invalidPermissions)) {
            $logger->log('warning', 'google_login_permissions_invalid', 'Google登录失败：请求的权限未授权', [
                'app_id' => $appId,
    ...
    if (!$config) {
        $logger->log('error', 'google_login_config_not_found', 'Google登录失败：Google登录配置未启用或不存在', ['app_id' => $appId]);
    ...
    $_SESSION['google_oauth_app_id'] = $appId;
    $_SESSION['google_oauth_callback_url'] = $callbackUrl;
    $_SESSION['google_oauth_permissions'] = $permissions;
    $_SESSION['google_oauth_state_code'] = $stateCode;
    
    // 记录日志
    $logger->log('info', 'google_login_start', 'Google登录发起授权请求', [
        'app_id' => $appId,
    ...
    $params = [
        ...
        'scope' => $scopes,
        'state' => $state,
        'access_type' => 'online',  // Google特有参数
        'prompt' => 'select_account'  // 每次都显示账号选择页面
    ];
