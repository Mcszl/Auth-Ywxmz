...
try {
    ...
    if ($method === 'phone') {
        ...
        $stmt = $pdo->prepare("
            SELECT template_id, purpose, is_enabled, status, daily_sent_count, daily_limit 
            FROM site_configs.sms_config 
            WHERE purpose IN ('change_email', '修改邮箱')
            AND is_enabled = TRUE 
            AND status = 1
        ...
        if (!$smsConfig) {
            ...
            ");
            $stmt->execute();
            $anyConfig = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($anyConfig) {
                // 有配置但不满足条件
                $reason = [];
                if (!$anyConfig['is_enabled']) {
                    $reason[] = '短信模板未启用';
                }
                if ($anyConfig['status'] != 1) {
                    $reason[] = '短信模板状态异常';
                }
                if ($anyConfig['daily_sent_count'] >= $anyConfig['daily_limit']) {
                    $reason[] = '今日发送量已达上限';
                }
                $reasonText = implode('，', $reason);
                
                error_log("修改邮箱短信配置不可用: " . $reasonText);
                $logger->log('change_email', 'sms_config_unavailable', $userUuid, [
                    'reason' => $reasonText,
                    'config' => $anyConfig
                ]);
                jsonResponse(false, null, '短信服务暂时不可用：' . $reasonText);
            } else {
                // 完全没有配置
                error_log("未找到修改邮箱短信配置");
                $logger->log('change_email', 'sms_config_not_found', $userUuid, [
                    'purpose' => 'change_email'
                ]);
                jsonResponse(false, null, '短信服务暂时不可用：未配置修改邮箱场景的短信模板');
            }
        }
        ...
        $rateLimitResult = $rateLimitService->checkRateLimit($target, $clientIp, $templateId, 'change_email');
        
        if (!$rateLimitResult['allowed']) {
            // 记录触发速率限制的日志
            $logger->log('change_email', 'rate_limit_triggered', $userUuid, [
                'phone' => $target,
    ...
    if ($method === 'phone') {
        // 生成短信ID
        $smsId = 'chgem_' . date('YmdHis') . '_' . substr(md5($target . time()), 0, 8);
        ...
        $stmt = $pdo->prepare("
            INSERT INTO sms.code (
                ...
                :sms_id, :phone, :code, 1, 'change_email',
        ...
        // 生成验证码ID
        $codeId = 'chgem_' . date('YmdHis') . '_' . substr(md5($target . time()), 0, 8);
    ...
    if ($method === 'phone') {
        ...
        $smsResult = $smsService->sendVerificationCode($target, 'change_email', 600, $clientIp);
        
        if (!$smsResult['success']) {
            $logger->log('change_email', 'sms_send_failed', $userUuid, [
                'phone' => $target,
        ...
        $logger->log('change_email', 'sms_sent', $userUuid, [
            'phone' => $target
        ]);

        // 记录发送（增加频率限制计数）
        $rateLimitService->recordSend($target, $clientIp, $templateId, 'change_email');
        ...
        $mailResult = $mailService->sendVerificationCode($target, $user['username'], $code, 'change_email', 10);
        
        if (!$mailResult['success']) {
            $logger->log('change_email', 'email_send_failed', $userUuid, [
                'email' => $target,
        ...
        $logger->log('change_email', 'email_sent', $userUuid, [
            'email' => $target
        ]);
